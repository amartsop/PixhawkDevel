...
int tricopter_control_thread_main(int argc, char *argv[])
{
	...
	warnx("[tricopter control] started");
	...
	/* initialize psi_previous */
	/* get a local copy of attitude */
	orb_copy(ORB_ID(vehicle_attitude), att_sub, &att);
	psi_previous = Measurements::estimate_psi(&att);
	...
	while (!thread_should_exit) {	
		...
		if (ret < 0) {
			...
			if (fds[1].revents & POLLIN) {
				...
				State::state_est(&state_vec, &measurements_vec, 
					&estimator_stat, &psi_previous); 
...
usage(const char *reason)
{
	...
	fprintf(stderr, "usage: tricopter_control {start|stop|status}\n\n");
}


int tricopter_control_main(int argc, char *argv[])
{
	if (argc < 2) {
	...
	if (!strcmp(argv[1], "start")) {
		...
		if (thread_running) {
			printf("tricopter_control already running\n");
			/* this is not an error */
			return 0;
		}

		thread_should_exit = false;
		deamon_task = px4_task_spawn_cmd("tricopter_control",
			SCHED_DEFAULT,
			SCHED_PRIORITY_MAX - 20,
			2048,
			tricopter_control_thread_main,
	...
	if (!strcmp(argv[1], "status")) {
		if (thread_running) {
			printf("\t tricopter_control is running\n");

		} else {
			printf("\t tricopter_control not started\n");
		}
